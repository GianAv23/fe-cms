...
type NewsCategoryDistribution = {
  category: "INTERNAL" | "EXTERNAL";
  count: number;
};

const chartData = [
  { category: "INTERNAL", count: 0, fill: "var(--chart-1)" },
  { category: "EXTERNAL", count: 0, fill: "var(--chart-2)" },
];
...
const NewsCategoryDistribution = () => {
  const { data: NewsCategoryDistribution } = useSWR<NewsCategoryDistribution[]>(
    `/news/stats/categorydistribution`,
  );

  const mappedData = NewsCategoryDistribution
    ? NewsCategoryDistribution.map((item: NewsCategoryDistribution) => ({
    ...
    <Card className="bg-sidebar gap-0 border-[0.5px] py-0 shadow-none">
      <CardHeader className="gap-1 rounded-t-xl px-3 py-4">
        ...
        <CardDescription className="text-xs lg:text-base">
          Show distribution of news based on category
        </CardDescription>
      </CardHeader>
...
export default NewsCategoryDistribution;
