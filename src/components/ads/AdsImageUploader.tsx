...
const AdsImageUploader: React.FC<ImageUploaderProps> = ({
  ads_uuid,
  onUploadSuccess,
  className,
  ...
  const handleSubmit = async (data: ImageUploaderForm) => {
    if (!ads_uuid) {
      toast.error("Ads UUID is required for uploading image.");
      return;
    }
    ...
    try {
      ...
      const response = await api.post<ResponseModel<string>>(
        `/ads-image/${ads_uuid}`,
        formData,
        {
          headers: {
