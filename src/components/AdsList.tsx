...
const AdsList = () => {
  ...
  const { data: Ads } = useSWR<Ads>(`/ads/ads-cms`);

  const events = Ads?.ads || [];
  ...
  return (
    <Card className="bg-sidebar gap-0 border-[0.5px] py-0 shadow-none">
      <CardHeader className="gap-0 rounded-t-xl px-3 pb-4">
        ...
        <div className="bg-primary-ads/10 text-primary-ads flex flex-1 flex-row items-center justify-between gap-4 rounded-sm px-2 py-1">
          <div className="flex flex-row items-center gap-2">
            <span className="text-sm font-semibold">Ads Created</span>
          </div>
        ...
        <div className="flex w-full flex-col gap-2">
          {Ads?.ads.slice(0, 5).map((ad) => (
            <div
              ...
              onClick={() => {
                navigate(`/dashboard/ads/${ad.uuid}` as Path, {});
              }}
            >
